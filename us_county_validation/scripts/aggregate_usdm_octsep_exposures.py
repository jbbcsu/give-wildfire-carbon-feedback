#!/usr/bin/env python3
"""Aggregate county USDM shares from preceding October through harvest September."""
...
def aggregate_state_harvest_year(
    weeks: pd.DataFrame, state: str, harvest_year: int, crops: tuple[str, ...]
) -> pd.DataFrame:
    start = pd.Timestamp(harvest_year - 1, 10, 1)
    end = pd.Timestamp(harvest_year, 9, 30)
    overlap = weeks.loc[weeks.valid_end.ge(start) & weeks.valid_start.le(end)].copy()
    if overlap.empty:
        raise ValueError(f"no USDM intervals overlap {state} harvest year {harvest_year}")
    ...
    if (overlap.days <= 0).any():
        raise ValueError(f"nonpositive clipped interval in {state} {harvest_year}")
    ...
    if ordered.loc[has_previous, "clip_start"].ne(expected_start.loc[has_previous]).any():
        raise ValueError(f"gapped or overlapping intervals in {state} {harvest_year}")
    ...
    if (
        ...
        raise ValueError(f"incomplete daily coverage in {state} {harvest_year}")
    ...
    if not np.allclose(sums.all_category_weeks, expected_weeks, rtol=0, atol=0.08):
        raise ValueError(f"category weeks do not reconcile in {state} {harvest_year}")
    ...
    for crop in crops:
        ...
        part.insert(3, "harvest_year", harvest_year)
        part.insert(4, "exposure_period", "previous_october_through_harvest_september")
        part["period_start"] = start
        part["period_end"] = end
        part["period_days"] = expected_days
...
def main() -> None:
    ...
    # The source archive begins one year before the first harvest-year outcome.
    harvest_years = range(int(contract["year_min"]) + 1, int(contract["year_max"]) + 1)
    ...
    try:
        for state in states:
            ...
            for harvest_year in harvest_years:
                result = aggregate_state_harvest_year(frame, state, harvest_year, crops)
                ...
                if writer is None:
                    ...
                    raise ValueError(f"output schema drift for {state} {harvest_year}")
                writer.write_table(output)
                total_rows += len(result)
                state_rows += len(result)
                state_counties.update(map(str, result.county_geoid.unique()))
            state_audit.append({"state": state, "rows": state_rows, "counties": len(state_counties)})
    finally:
        if writer is not None:
            writer.close()
    if writer is None:
        raise RuntimeError("no October--September exposures written")
    metadata = pq.ParquetFile(arguments.out).metadata
    if metadata.num_rows != total_rows:
        raise RuntimeError("October--September exposure Parquet row count mismatch")
    audit = {
        "schema": "usdm_octsep_county_exposure_v1",
        ...
        "states": state_audit,
        "crops": list(crops),
        "harvest_year_start": min(harvest_years),
        "harvest_year_end": max(harvest_years),
        "exposure_definition": "sum of county-area category fraction times covered days divided by seven, October 1 of previous year through September 30 of harvest year",
        "category_reconciliation_tolerance_weeks": 0.08,
    ...
    print(f"wrote {total_rows} crop-county-harvest-year October--September exposure rows")
