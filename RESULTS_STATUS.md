...
| Full-GIVE replacement build gate | The installation harness removes legacy MooreAg agriculture, reuses GIVE's regional socioeconomic aggregators, preserves declared sector flags, passes the graph audit, and builds the unmodified GIVE model with six crops and synthetic full-time-axis zero-response inputs | Topology/build compatibility only; synthetic shares and zero coefficients are not empirical inputs, and no model run, marginal damage, welfare, discount, or SCC result is created |
