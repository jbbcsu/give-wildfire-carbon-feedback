...
| Predeclared later-century ISIMIP3b support expansion | The live official API validates a full 5-ESM x 3-SSP x 2-variable x 2-period Cartesian product: 30 version-`20210512` datasets and 60 public, unrestricted CC0 files totaling 124,935,312,957 bytes. The fixed blocks are 2041--2050 and 2091--2100; eligible engineering harvest years are 2042--2049 and 2092--2099 so cross-year seasons never cross an unacquired boundary. Both registered GFDL-ESM4 SSP1-2.6 `pr`/`tas` pairs now pass full SHA-512 and decoded 3,652-day global-grid content validation. Each variable/block has 946,598,400 finite values and zero missing values; both `pr` blocks have zero negative values. Same-realization `tas` produces ten annual GMST rows per block. Each bounded two-latitude-row maize/rainfed smoke produces 5,488 seasonal and 16,464 stage rows with exact additive stage reconciliation. | Four of 60 complete-file gates and two bounded feature blocks only. Joined holdouts and support effects remain unvalidated. The blocks are noncontiguous, and FAIR after 2100 remains outside direct ISIMIP training support. |
