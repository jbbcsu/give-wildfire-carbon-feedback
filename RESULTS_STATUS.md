...
| U.S. all-practice national county-weight expansion | The registered all-county launcher validates and resumes 932 of 2,628 isolated county-weight receipts, then fails closed at Trigg County, Kentucky (21221). Exact recomputation gives full geometric grid coverage but only 0.907267979 weather-valid area relative to declared land, below the fixed 0.95 gate; 209,051,009 m2 of polygon intersection is masked. The same 77 valid cells and area terms recur exactly in January 1981, July 2000, and January 2019, and separate `prcp`, `tavg`, `tmin`, and `tmax` masks reproduce that exact result. | Reproducible structural-coverage blocker only. The threshold is unchanged, no Trigg partition is written, and no partial national feature panel, response, damage, or SCC input is authorized. Audit the common source mask/land denominator and preregister any exclusion or sensitivity rule before resuming. |
