...
| U.S. all-practice national county-weight expansion | The registered all-county launcher validates and resumes 932 of 2,628 isolated county-weight receipts, then fails closed at Trigg County, Kentucky (21221). Exact recomputation gives full geometric grid coverage but only 0.907267979 weather-valid area relative to declared land, below the fixed 0.95 gate; 209,051,009 m2 of polygon intersection is masked. The same 77 valid cells and area terms recur exactly in January 1981, July 2000, and January 2019. | Reproducible structural-coverage blocker only. The threshold is unchanged, no Trigg partition is written, and no partial national feature panel, response, damage, or SCC input is authorized. Audit the source mask/land denominator and preregister any exclusion or sensitivity rule before resuming. |
...
| Predeclared later-century ISIMIP3b support expansion | The live official API validates a full 5-ESM x 3-SSP x 2-variable x 2-period Cartesian product: 30 version-`20210512` datasets and 60 public, unrestricted CC0 files totaling 124,935,312,957 bytes. The fixed blocks are 2041--2050 and 2091--2100; eligible engineering harvest years are 2042--2049 and 2092--2099 so cross-year seasons never cross an unacquired boundary. Both registered GFDL-ESM4 SSP1-2.6 `pr`/`tas` pairs, both SSP3-7.0 pairs, and the SSP5-8.5 2041--2050 pair pass full SHA-512, decoded 3,652-day global-grid, same-realization GMST, and bounded-feature gates. Each variable/block has 946,598,400 finite values and zero missing values; every `pr` block has zero negative values. Each two-latitude-row maize/rainfed smoke produces 5,488 seasonal and 16,464 stage rows with exact additive reconciliation. The joined three-SSP midcentury product has 181,104 rows. Whole-scenario GMST adjustment improves 14/33 feature comparisons versus a cell-mean benchmark (median ratio 1.00036; maximum 1.06410), only 1/11 for held-out SSP5-8.5, and 20,562/181,104 held-out values (11.35%) lie outside the exact two-scenario cell/feature envelope. Temperature-only reclassification of paired FAIR paths against the 287.659--290.514 K GFDL envelope moves the last within-envelope baseline year from 2020 to 2221 while pairing, identity, and decreasing-pulse gates remain passed. | Ten of 60 complete-file gates and five bounded feature blocks only. The adverse scenario holdout rejects promotion. Support flags describe held-out climate features, not FAIR baseline/pulse features; whole-ESM, response, damage, and SCC gates remain open. The blocks are noncontiguous, and FAIR after 2100 remains outside direct ISIMIP training support. |
