...
| RIME-X v1.0, Schwind et al. (2026), https://doi.org/10.5194/gmd-19-6797-2026; exact paper archive https://doi.org/10.5281/zenodo.21061984 | Links simple-climate-model warming distributions to gridded or regional CMIP/ISIMIP climate and impact indicators through 0.1 K warming-level conditional distributions, 101 quantiles, and linear interpolation; includes held-out-scenario validation. | Closest published direct indicator-response benchmark for daily-derived crop features; its version-pinned GIVE contract and synthetic pulse smoke are in `RIMEX_FEATURE_RESPONSE_BENCHMARK.md`. A preregistered GFDL/SSP1-2.6 2031--2060 pilot would supply eight valid centered outputs. | The published application uses 21-year smoothing, while the current bounded GIVE feature panel has discontinuous short blocks. The pinned contiguous pilot still lacks four acquired/validated files, other ESMs/scenarios, and a validated joint-dependence design. Published quantile maps are univariate. Real fitting and FAIR evaluation remain closed. |
