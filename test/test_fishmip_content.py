...
from validate_fishmip_content import EXPECTED_LAT, EXPECTED_LON, TIME_CALENDAR, TIME_UNITS, validate


def write(path: Path, values: np.ndarray, *, units: str = "g m-2") -> tuple[int, str]:
    dataset = xr.Dataset(
        {"tc": (("time", "lat", "lon"), values, {"units": units})},
        coords={
            "time": ("time", np.arange(5028, 5040, dtype=float), {"units": TIME_UNITS, "calendar": TIME_CALENDAR}),
            "lat": EXPECTED_LAT,
            "lon": EXPECTED_LON,
        },
    )
...
def row(path: Path, size: int, digest: str) -> dict[str, str]:
    return {
        "dataset_id": "dataset",
        "file_id": "file",
        "model": "boats",
