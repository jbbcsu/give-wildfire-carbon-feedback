...
STAGE_CONSTANTS = [
    "cross_year", "stage_id", "stage_start_offset_day",
...
def smooth_features(frame: pd.DataFrame, *, stage: bool, window: int) -> pd.DataFrame:
    ...
    required = {"harvest_year", "plant_year", *group_keys, *constants, *METRICS}
    ...
    if not np.isfinite(frame[METRICS].to_numpy(dtype=float)).all():
        raise ValueError("feature metrics must be complete and finite before smoothing")
    ...
    for key, group in frame.groupby(group_keys, observed=True, sort=True):
        ...
        for center in range(first_year + half, last_year - half + 1):
            ...
            row.update({f"{name}_{window}yr_mean": float(block[name].mean()) for name in METRICS})
            rows.append(row)
...
def reconcile(season: pd.DataFrame, stage: pd.DataFrame, gmst: pd.DataFrame, *, window: int) -> dict[str, object]:
    ...
    limits = {"precip_mm": 1e-9, "wet_days_n": 1e-12}
