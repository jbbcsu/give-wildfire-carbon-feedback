#!/usr/bin/env python3
"""Joint bounded three-ESM/four-scenario feature holdout audit."""
...
CONFIG_SCHEMA = "isimip3b_bounded_three_esm_four_scenario_holdout_config_v1"
CONFIG_ROLE = "outcome_blind_joint_whole_esm_and_whole_scenario_engineering_smoke_not_complete_emulator_damage_or_scc_input"
EXPECTED_ESMS = {"GFDL-ESM4", "IPSL-CM6A-LR", "MPI-ESM1-2-HR"}
...
def assemble(config_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if config.get("schema") != CONFIG_SCHEMA or config.get("role") != CONFIG_ROLE:
        raise ValueError("three-ESM joint holdout contract identity changed")
    ...
    for gate in (
        ...
        if limits.get(gate) is not False:
            raise ValueError(f"three-ESM joint holdout unexpectedly opens {gate}")
    products = config.get("training_products", [])
    if len(products) != 3 or {str(row.get("esm_id")) for row in products} != EXPECTED_ESMS:
        raise ValueError("joint holdout lacks the exact three ESM products")
...
def main() -> None:
    ...
    audit = {
        "schema": "isimip3b_bounded_three_esm_four_scenario_holdout_v1", "role": CONFIG_ROLE,
        **metadata,
        "implementation": {
        ...
        "limitations": [
            "Only three of the five frozen ESM realizations have the exact four-scenario bounded product.",
    ...
    print(
        f"joint three-ESM/four-scenario holdout passed: {len(training)} rows, "
