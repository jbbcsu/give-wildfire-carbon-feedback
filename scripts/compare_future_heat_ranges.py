...
def diagnose(history,future):
    ...
    if not np.isfinite(history[HEAT].to_numpy()).all() or not np.isfinite(future[HEAT].to_numpy()).all():
        raise ValueError('nonfinite heat')
    groups=history.groupby(GRID)
    lo=groups[HEAT].min();hi=groups[HEAT].max();count=groups.size()
...
def main():
    ...
    if parent['status']!='two_crop_joint_climate_inputs_validated':raise ValueError('unvalidated future inputs')
    result=dict(role='marginal_heat_ranges_not_transport_or_damages',crop_yield_estimated=False,
        causal_or_scc_result=False,soybean_29c_is_not_locked_30c_control=True,
        threshold_c=29,minimum_years=2,absolute_tolerance=TOL,
    ...
    for crop,label in [('mai','maize'),('soy','soy')]:
        ...
        for batch in pq.ParquetFile(path).iter_batches(batch_size=8192,columns=KEYS+['yield_t_ha']+HEAT,use_threads=False):
        ...
        result['comparisons'].append(dict(crop=crop,esm=product['esm'],scenario=product['scenario'],
            ...
            future_sha256=product['sha256'],**diagnose(history,future)))
