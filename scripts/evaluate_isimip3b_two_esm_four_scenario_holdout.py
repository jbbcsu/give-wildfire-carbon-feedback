#!/usr/bin/env python3
"""Joint bounded whole-ESM and whole-scenario feature holdout audit."""
...
CONFIG_SCHEMA = "isimip3b_bounded_two_esm_four_scenario_holdout_config_v1"
CONFIG_ROLE = "outcome_blind_joint_whole_esm_and_whole_scenario_engineering_smoke_not_complete_emulator_damage_or_scc_input"
EXPECTED_ESMS = {"GFDL-ESM4", "IPSL-CM6A-LR"}
EXPECTED_SCENARIOS = {"historical", "ssp126", "ssp370", "ssp585"}


def read_source_audit(path: Path, esm_id: str, training_sha256: str) -> dict[str, Any]:
    audit = json.loads(path.read_text(encoding="utf-8"))
    if audit.get("result") != "passed" or audit.get("esm_id") != esm_id:
        raise ValueError(f"{esm_id} source scenario audit did not pass")
    if "damage_or_scc_authorized" in audit:
        if audit.get("damage_or_scc_authorized") is not False:
            raise ValueError(f"{esm_id} source audit opens damage/SCC use")
    else:
        role = str(audit.get("role", ""))
        limitations = " ".join(map(str, audit.get("limitations", [])))
        if "not_complete_emulator_damage_or_scc_input" not in role or "No paired baseline/pulse" not in limitations:
            raise ValueError(f"{esm_id} legacy source audit lacks a closed damage/SCC boundary")
    if audit.get("training_output", {}).get("sha256") != training_sha256:
        raise ValueError(f"{esm_id} source audit training hash changed")
    return audit
...
def assemble(config_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if config.get("schema") != CONFIG_SCHEMA or config.get("role") != CONFIG_ROLE:
        raise ValueError("joint holdout contract identity changed")
    ...
    for gate in (
        ...
        if limits.get(gate) is not False:
            raise ValueError(f"joint holdout unexpectedly opens {gate}")
    products = config.get("training_products", [])
    if len(products) != 2 or {str(row.get("esm_id")) for row in products} != EXPECTED_ESMS:
        raise ValueError("joint holdout lacks the exact two ESM products")
    frames: list[pd.DataFrame] = []
    receipts: list[dict[str, Any]] = []
    for product in products:
        esm_id = str(product["esm_id"])
        member_id = str(product["member_id"])
    ...
    if set(combined["esm_id"].astype(str)) != EXPECTED_ESMS:
        raise ValueError("joint holdout ESM set changed")
    if set(combined["scenario"].astype(str)) != EXPECTED_SCENARIOS:
        raise ValueError("joint holdout scenario set changed")
    ...
    spatial = {
        esm_id: frame[CELL_KEYS].drop_duplicates().sort_values(CELL_KEYS).reset_index(drop=True)
        for esm_id, frame in combined.groupby("esm_id", sort=True)
    }
    if not spatial["GFDL-ESM4"].equals(spatial["IPSL-CM6A-LR"]):
...
def summarize(esm_holdouts: pd.DataFrame, scenario_holdouts: pd.DataFrame) -> dict[str, Any]:
    products = {"esm": esm_holdouts, "scenario": scenario_holdouts}
    result: dict[str, Any] = {}
    for split, frame in products.items():
        if set(frame.get("split_type", pd.Series(dtype=str)).astype(str)) != {split}:
            raise ValueError(f"{split} holdout split identity changed")
        numeric = frame[["rmse", "benchmark_rmse"]].apply(pd.to_numeric, errors="coerce")
        if numeric.isna().any().any() or (numeric < 0).any().any() or (numeric["benchmark_rmse"] == 0).any():
            raise ValueError(f"{split} holdout errors are invalid")
        ratios = numeric["rmse"] / numeric["benchmark_rmse"]
        result[split] = {
            "comparisons": int(len(frame)),
            "gmst_model_better_count": int((numeric["rmse"] < numeric["benchmark_rmse"]).sum()),
            ...
            "holdouts": {
                ...
                for holdout, block in frame.groupby("holdout_id", sort=True)
            },
        }
    return result
...
def main() -> None:
    ...
    audit = {
        "schema": "isimip3b_bounded_two_esm_four_scenario_holdout_v1",
        "role": CONFIG_ROLE,
        **metadata,
        "implementation": {
        ...
        "limitations": [
            "Only two of the five frozen ESM realizations have the exact four-scenario bounded product.",
    ...
    print(
        f"joint two-ESM/four-scenario holdout passed: {len(training)} rows, "
