#!/usr/bin/env python3
"""One-tile four-year fallback after the eight-year 512 MiB guard stopped."""
...
from run_ukesm_eight_year_tile_pilot import ROOT, CALENDAR, source_context, digest, require, run


OUT = ROOT / "data/interim/ukesm_ssp126_2092_2095_mai_noirr_lat100_110_pilot_20260917"
...
def main() -> None:
    ...
    for label, builder in (("season", "build_crop_year_features.py"),
        ...
        output, receipt, log = (OUT / f"{label}.{suffix}" for suffix in
                                ("parquet", "resource.json", "log"))
        ...
        paths[label], peaks[label] = output, result["sampled_peak_group_rss_bytes"]
    manifest = {
        "schema": "ukesm_ssp126_2092_2095_mai_noirr_lat100_110_pilot_v1",
        ...
        "harvest_years": list(range(2092, 2096)),
    ...
    print(json.dumps({"status": manifest["status"], "season_rows": manifest["season_rows"],
                      "stage_rows": manifest["stage_rows"],
                      "peak_rss": manifest["peak_sampled_worker_rss_bytes"]}, sort_keys=True))
